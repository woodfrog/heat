...
if __name__ == '__main__':
    ...
    pg_base = './npy_floorplan_th5/'
    # pg_base = './letr_npy_floorplan/s3d_256_query_100_stage2_focal'
    # pg_base = './hawp_npy_floorplan_post/'
    # pg_base = '../../caption_editing/geometric-learning/geometric-ssl/data/s3d_floorplan/annot/'
    for filename in sorted(os.listdir(pg_base)):
        # if filename in ['03268.npy', '03270.npy', '03290.npy',
        #                 '03307.npy', '03336.npy', '03341.npy', '03343.npy',
        #                 '03349.npy', '03358.npy', '03361.npy', '03363.npy', '03365.npy',
        #                 '03369.npy', '03376.npy', '03421.npy', '03433.npy', '03495.npy']:
        #     continue
        # if filename != '03435.npy':
        #     continue

        # file_id = int(filename[:-4])
        # if file_id < 3250:
        #     continue
        # annot_path = os.path.join(pg_base, filename)
        # annot = np.load(annot_path, allow_pickle=True).tolist()
        # example_pg = convert_annot(annot)

        pg_path = os.path.join(pg_base, filename)
        example_pg = np.load(pg_path, allow_pickle=True).tolist()
        # example_pg = cleanup_pg(example_pg)
        ...
        #     cv2.line(image, tuple(corners[edge_pair[0]]), tuple(corners[edge_pair[1]]), (255, 255, 0), 2)
        #
        # for c_idx, c in enumerate(corners):
        #     cv2.circle(image, (int(c[0]), int(c[1])), 3, (0, 255, 0), -1)
        #     cv2.putText(image, '{}'.format(c_idx), (c[0] - 5, c[1]), cv2.FONT_HERSHEY_SIMPLEX,
        #                 0.5, (0, 0, 255), 1, cv2.LINE_AA)
        # cv2.imwrite('./debug_heat_th5/debug_{}.png'.format(filename[:-4]), image)
        ...
        cv2.imwrite('./viz_heat_th5/{}.png'.format(filename[:-4]), floorplan_image)
