...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None,
        ...
        self.bn1 = BatchNorm(planes, track_running_stats=False)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes,
                             padding=dilation[1], dilation=dilation[1])
        self.bn2 = BatchNorm(planes, track_running_stats=False)
...
class DRN(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000,
        ...
        if arch == 'C':
            ...
            self.bn1 = BatchNorm(channels[0], track_running_stats=False)
            self.relu = nn.ReLU(inplace=True)
            ...
            self.layer0 = nn.Sequential(
                nn.Conv2d(3, channels[0], kernel_size=7, stride=1, padding=3,
                          bias=False),
                BatchNorm(channels[0], track_running_stats=False),
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1,
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                BatchNorm(planes * block.expansion, track_running_stats=False),
            )
...
def drn_c_26(pretrained=False, **kwargs):
    ...
    if pretrained:
        pretrained_dict = model_zoo.load_url(model_urls['drn-c-26'], model_dir='./.torch/checkpoints')
